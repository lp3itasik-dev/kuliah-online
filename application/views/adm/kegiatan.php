...
<body>
									...
									<table class="table table-framed" id="datatable-basic">
										...
										<tbody>
										    ...
										    foreach($read as $r){
										            ...
										            if($r->jenis=="9"){$jenis="Presensi LP3I Group";}
													...
													<tr>
														...
														<td width="50px" class="text-center">
															<button class="btn btn-success" onclick="return ubah(`'.$r->id.'`,`'.$r->kegiatan.'`,`'.$r->tanggal.'`,`'.$r->dari.'`,`'.$r->sampai.'`,`'.$r->jenis.'`,`'.$r->tanggal_daftar.'`,`'.$jenis.'`)">EDIT</button>
															<button class="btn btn-danger" onclick="return hapus(`'.$r->id.'`,`'.$r->kegiatan.'`)">HAPUS</button>
															<a class="btn bg-lp3i" href="'.base_url().'adm/laporan/'.$r->id.'" target="_blank">CETAK</a>
														</td>
													</tr>
													';
												}
	...
	<script>
        ...
        function tambah(){
            ...
            $('#formId').attr('action', '<?= base_url() ?>adm/add_kegiatan');
        ...
        function ubah(id,nama,tanggal,dari,sampai,jenis,tanggal_daftar,jenisname){
			$('#myModal').modal({backdrop: 'static', keyboard: false});
            ...
            $('.hapus').addClass('d-none');
            $('.input').removeClass('d-none');
            $('#formId').attr('action', '<?= base_url() ?>adm/update_kegiatan/'+id);
            $('[name="nama"]').attr('required','');
            $('[name="tanggal"]').attr('required','');
            $('[name="dari"]').attr('required','');
            $('[name="sampai"]').attr('required','');
        ...
        function hapus(id,nama){
            ...
            $('#formId').attr('action', '<?= base_url() ?>adm/delete_kegiatan/'+id);
						...
						<div class="form-group row">
							...
							<div class="col-lg-9">
								<select name="jenis" onchange="return jenis_pendaftaran()" class="form-control select-search" required>
								    ...
								    <option value="3">Presensi (Karyawan)</option>
								    <option value="5">Presensi (Mahasiswa)</option>
								    <option value="9">Presensi LP3I Group</option>
